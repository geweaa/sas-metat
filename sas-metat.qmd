...
theme_tidy <- function(fontsize = 6, axisface = "plain", legend = "right") {
  theme(
    # Text elements
    axis.title.x = element_text(colour = "black", size = fontsize, face = axisface),
    axis.title.y = element_text(colour = "black", size = fontsize, face = axisface, angle = 90),
    axis.text.x = element_text(colour = "black", size = fontsize, face = axisface),
    axis.text.y = element_text(colour = "black", size = fontsize, face = axisface),
    # Legend elements
    legend.position = legend,
    #legend.box.background = element_blank(),
    legend.text = element_text(colour = "black", size = fontsize),
    legend.title = element_text(size = fontsize, colour = "black", hjust = 0),
    ...
    strip.text = element_text(colour = "black", size = fontsize, hjust = 0.5)
  )
}
...
genelist <- read_tsv('data/genelist-merged.tsv.gz', col_types = 'cccc')
```
...
emapper <- read_tsv("data/metatdenovo/emapper-cogs.tsv.gz", col_types = 'ccc')
```
...
kos <- kofam %>% 
  ...
  inner_join(genelist, by = 'ko', relationship = 'many-to-many') %>% filter(category == 'Metabolism') %>% 
  left_join(tax, by = "orf") %>%
  ...
  # Sum the tpm 
  group_by(pathway, subcategory, layer, order) %>% summarise(tpm = sum(tpm), .groups = "drop")
```
...
p1 <- kos %>% 
  filter(subcategory == "Central C metabolism") %>%
...
p2 <- kos %>% 
  filter(subcategory == "N metabolism") %>%
  rbind(i) %>%
...
p3 <- kos %>% 
  filter(subcategory == "P metabolism") %>%
  ggplot(aes(
    x = tpm, 
    y = paste(pathway, layer) %>% fct_rev(),
    fill = fct_relevel(order, names(cols.ko))
...
kos <- genelist %>%
  ...
  inner_join(kofam, by = 'ko') %>% 
  inner_join(tax, by = "orf") %>% 
  inner_join(tpm, by = "orf") %>% 
  inner_join(meta, by = "sample") %>%
  replace_na(list("order" = "Other")) %>%
  mutate(order = if_else(order %in% names(cols.order), order, "Other")) %>%
  group_by(layer, pathway, order, subcategory) %>% summarise(tpm = sum(tpm), .groups = "drop") %>%
  mutate(order = factor(order, names(cols.order))) %>%
  mutate(i = if_else(
     layer == "10m", true = pathway, false = paste(pathway, layer)
   ))
```
...
```{r}
lab <- kos %>%
  filter(subcategory == "EPS biosynthesis") %>%
  arrange(i) %>%
  select(i) %>%
  distinct() %>%
...
```{r}
lab <- kos %>%
  filter(subcategory == "Vesicle formation") %>%
  arrange(i) %>%
  select(i) %>%
  distinct() %>%
  mutate(i = if_else(grepl("chlMax", i), "", i)) %>%
...
```{r}
lab <- kos %>%
  filter(subcategory == "External appendages and membrane extensions") %>%
  arrange(i) %>%
  select(i) %>%
  distinct() %>%
  mutate(i = if_else(grepl("chlMax", i), "", i)) %>%
...
```{r}
d <- genelist %>%
  filter(!category %in% c('Metabolism', 'Morphology')) %>% 
...
# Comparison with Tara Oceans

```{r}
#| label: import-tara

samples <- read_tsv('data/metatdenovo/tara-meta.tsv', col_types = 'cdc')

tara <- read_tsv('data/metatdenovo/tara-kos-metat.tsv.gz', col_types = 'ccd')

md <- meta %>% 
  mutate(region = 'CAO') %>% 
  select(sample, depth, region) %>% 
  rbind(samples) %>% 
  mutate(region = case_when(
    grepl('South Atlantic Ocean', region) ~ 'SAO',
    grepl('North Atlantic Ocean', region) ~ 'NAO',
    grepl('Arctic Ocean', region) ~ 'AO',
    .default = 'CAO'
  ))
```
...
```{r}
#| label: nmds-comparison

set.seed(999)

nmds <- tpm %>% 
  inner_join(kofam, by = 'orf') %>%
  select(sample, ko) %>%
  rbind(tara %>% select(sample, ko)) %>%
  # Occurrence of each KO within each sample
  group_by(sample, ko) %>% summarise(n = n(), .groups = 'drop') %>% 
  # Typical transformation for vegan library
  pivot_wider(names_from = 'ko', values_from = 'n', values_fill = 0) %>% 
  column_to_rownames('sample') %>% 
  vegan::metaMDS()

scores <- vegan::scores(nmds)$sites %>%
  as.data.frame() %>%
  rownames_to_column('sample') %>% 
...
```{r}
ggsave('figures/comparison-tara.pdf', width = 12, height = 12, units = 'cm')
```
