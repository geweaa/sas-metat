...
# Add the output of running Diamond again Prokka-annotated genomes
tax <- read_tsv('data/n.tsv', col_names = F, col_types = 'cc') %>%
  rename(orf = 1, tax = 2) %>% 
  separate(tax, into = c('domain','phylum','class','order','family','genus','species'), sep = ';') %>% 
  mutate(across(-1, \(x) gsub(pattern = '[a-z]__', replacement = '',x))) %>%
  rbind(tax)
```
